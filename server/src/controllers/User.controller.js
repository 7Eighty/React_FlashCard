...
class UserController {
  static async getAll(req, res) {
    try {
      const result = await UserServise.getAllUsers();
      res.status(200).json(
        formatResponse({
          statusCode: 200,
...
// ^ ищет по id -  сменить на name
 static async getOne(req, res) {
    try {
      ...
      const user = await UserService.getOneUser(nameUser)
      res.status(200)
      .json(formatResponse({
      ...
      res.status(502)
      .json(formatResponse({
        statusCode: 502, message: 'Не удалось получить пользователя',
        error: error.message
      }))
    }
  }
 ...
 static async register(req, res) {
    try {
        const {isValid, error} = await UserService.validate({
            if (!isValid) {
                res.status(400).json(formatResponse({
          statusCode: 400, message: 'Регистрация не прошла',
          error
        }))
            } else {
    ...
    res.status(503)
    .json(formatResponse({
        statusCode: 503, message: 'Не удалось создать пользователя',
        error: error.message
      }))
    }
